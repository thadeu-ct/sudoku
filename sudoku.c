...
int resolver_sudoku(int jogo[N][N]) {
    for (int linha = 0; linha < N; linha++) {
        for (int coluna = 0; coluna < N; coluna++) {
            if (jogo[linha][coluna] == 0) { /* para cada linha/coluna que esteja vazia */
                ...
                return 0; /* se nenhum número for válido, retorna falso */
            }
        }
    }
    return 1; /* se todas as posições foram preenchidas, o Sudoku está resolvido */
}

int eh_valido(int jogo[N][N], int linha, int coluna, int num){
    for (int i = 0; i < N; i++) {
    ...
    return 1; /* se não encontrou o número em nenhuma linha, coluna ou quadrante, é válido */
}
